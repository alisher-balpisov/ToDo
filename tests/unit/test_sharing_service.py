...
class TestSharingService:
    ...
    # FIX: Convert tests to async and await coroutines
    async def test_is_sharing_with_self_with_same_ids_returns_true(self):
        owner_id, target_id = 1, 1
        assert await is_sharing_with_self(owner_id, target_id) is True

    async def test_is_sharing_with_self_with_different_ids_returns_false(self):
        owner_id, target_id = 1, 2
        assert await is_sharing_with_self(owner_id, target_id) is False

    async def test_is_already_shared_for_unshared_task_returns_false(self, db_session, test_task, test_user2):
        result = await is_already_shared(db_session, test_user2.id, test_task.id)
        assert result is False

    async def test_is_already_shared_for_shared_task_returns_true(self, db_session, shared_task, test_user2):
        result = await is_already_shared(db_session, test_user2.id, shared_task.id)
        assert result is True

    async def test_share_task_service_with_valid_data_succeeds(self, db_session, test_user, test_user2, test_task):
        ...
        shared_task_check = await get_user_shared_task(db_session, test_user2.id, test_task.id)
        assert shared_task_check is not None
        assert shared_task_check.id == test_task.id

    async def test_share_task_service_to_nonexistent_user_raises_not_found(self, db_session, test_user, test_task):
    ...
    async def test_share_task_service_for_already_shared_task_raises_already_exists(self, db_session, test_user, test_user2, shared_task):
    ...
    async def test_share_task_with_self_raises_invalid_operation(self, db_session, test_user, test_task):
    ...
    async def test_get_permission_level_returns_correct_level(self, db_session, test_user2, shared_task):
        """Тест получения уровня разрешений для расшаренной задачи."""
        # Arrange (shared_task has 'edit' permission for test_user2)

        # Act
        permission = await get_permission_level(

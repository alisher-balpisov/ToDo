...
class TestTaskService:
    ...
    def test_create_task_with_valid_data_succeeds(self, db_session, test_user):
    ...
    def test_create_task_with_empty_or_none_name_raises_missing_field(self, db_session, test_user, name):
    ...
    def test_get_tasks_service_returns_user_tasks(self, db_session, test_user):
        """Тест получения списка задач должен вернуть все задачи пользователя."""
        # Arrange
        for i in range(3):
            create_task_service(db_session, test_user.id, f"Task {i}", "")

        # Act
        tasks = get_tasks_service(db_session, test_user.id, [], 0, 10)

        # Assert
        assert len(tasks) == 3
        assert all(task.user_id == test_user.id for task in tasks)

    def test_get_task_service_for_existing_task_returns_task(self, db_session, test_user, test_task):
        """Тест получения существующей задачи по ID должен вернуть объект задачи."""
        # Arrange
        task_id = test_task.id

        # Act
        task = get_task_service(db_session, test_user.id, task_id)
    ...
    def test_get_task_service_for_nonexistent_task_raises_not_found(self, db_session, test_user):
        ...
        with pytest.raises(ResourceNotFoundException) as exc_info:
            get_task_service(db_session, test_user.id, task_id)
    ...
    def test_get_task_service_for_other_user_task_raises_not_found(self, db_session, test_user2, test_task):
        ...
        # Act & Assert
        with pytest.raises(ResourceNotFoundException) as exc_info:
            get_task_service(db_session, test_user2.id, task_id)
        assert exc_info.value.error_code == "RESOURCE_NOT_FOUND"
        assert exc_info.value.resource_id == str(task_id)

    def test_update_task_service_with_valid_data_succeeds(self, db_session, test_user, test_task):
    ...
    def test_delete_task_service_for_existing_task_succeeds(self, db_session, test_user, test_task):
        """Тест успешного удаления существующей задачи."""
        # Arrange
        task_id = test_task.id

        # Act
        delete_task_service(db_session, test_user.id, task_id)

...
class TestAuthService:
    ...
    async def test_get_user_by_username_for_existing_user_returns_user(self, db_session, test_user):
    ...
    async def test_get_user_by_username_for_nonexistent_user_returns_none(self, db_session):
    ...
    async def test_register_service_with_valid_data_creates_user(self, db_session):
    ...
    async def test_get_current_user_with_valid_token_returns_user(self, db_session, test_user):
        """Тест получения текущего пользователя с валидным токеном."""
        # Arrange
        token = test_user.token()

        # Act
        current_user = await get_current_user(db_session, token)

        # Assert
        assert current_user.id == test_user.id
        assert current_user.username == test_user.username

    async def test_get_current_user_with_invalid_token_raises_invalid_credentials(self, db_session):
    ...
    async def test_get_current_user_with_expired_token_raises_token_expired(self, db_session, test_user):

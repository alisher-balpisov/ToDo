...
def update_task_by_id(
    ...
    try:
        ...
        task.completion_status = task_update.completion_status if task_update.completion_status else task.completion_status
...
def delete_task(id: int = Query(ge=1), current_user: User = Depends(get_current_active_user)):
    try:
        task = session.query(ToDo).filter(
            ToDo.id == id,
            ToDo.user_id == current_user.id
